...
class MessageDeletedEventTest extends TestCase
{
    ...
    /**
     * @dataProvider provideSnowflake
     * @param $messageId
     */
    public function testSetMessageId($messageId)
    {
        $event = MessageDeletedEvent::setMessageId($messageId);
        $this->assertEquals($messageId, $event->getId());
        $this->assertEquals($messageId, $event->getMessageId());
    }

    public function provideSnowflake()
    {
        $this->setupFaker();
